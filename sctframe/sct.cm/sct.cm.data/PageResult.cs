...
namespace sct.cm.data
{
    ...
    public class PageResult<T> : IEnumerable<T>, ICollection<T>
    {
        #region Public Fields
        /// <summary>
        /// 获取一个当前类型的空值。
        /// </summary>
        public static readonly PageResult<T> Empty = new PageResult<T>(0, 0, 0, 0, null);
        #endregion

        #region Ctor
        /// <summary>
        /// 初始化一个新的<c>PageResult{T}</c>类型的实例。
        /// </summary>
        public PageResult()
        {
            Data = new List<T>();
        ...
        public PageResult(int totalRecords, int totalPages, int pageSize, int pageNumber, List<T> data)
        {
