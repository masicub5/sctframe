...
namespace sct.svc.uc.imp
{
    ...
    public  class FunctionService : FunctionBaseService,IFunctionService
    {


        public PageResult<FunctionInfo> ListByCondition(NameValueCollection searchCondtionCollection, NameValueCollection sortCollection, int pageNumber, int pageSize)
        {

            PageResult<FunctionInfo> result = new PageResult<FunctionInfo>();
            ...
            using (var DbContext = new UCDbContext())
            {
                ...
                foreach (string key in searchCondtionCollection)
                {
                    ...
                    {
                        case "functionname":
                            query = query.Where(x => x.FunctionName.Contains(condition));
                            break;
                        case "functioncode":
                            query = query.Where(x => x.FunctionCode.Contains(condition));
                            break;
                        case "isvalid":
                ...
                foreach (string sort in sortCollection)
                {
                    ...
                    switch (sort.ToLower())
                    {
                        ...
                        case "functionname":
                            if (direct.ToLower().Equals("asc"))
                            {
                                query = query.OrderBy(x => x.FunctionName).Skip(skip).Take(take);
                            }
                            else
                            {
                                query = query.OrderByDescending(x => x.FunctionName).Skip(skip).Take(take);
                            }
                            break;
                        default:
