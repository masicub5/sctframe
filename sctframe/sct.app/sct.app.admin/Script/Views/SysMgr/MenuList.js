$(function () {
    ...
    PageClick = function (pageclickednumber) {
        ...
        var parentid = $("#drplMenu").val();
        var appid = $("#drplApp").val();
        var isvalid = $("#drplIsvalid").val();
        param = { name: name, parentid: parentid, appid: appid, isvalid: isvalid, pagenumber: pageclickednumber, pagesize: pagesize };

        InvokeAction.InvokeJsonAction('SysMgr', 'ListMenu', param, function (jsonData) {
            ...
            if (jsonData != null && jsonData.rows != null && jsonData.rows.length > 0) {
                for (i = 0 ; i < jsonData.rows.length; i++) {
                    ...
                    htmllist += '<td><input type = "checkbox" name ="chkSelect"  value="' + jsonData.rows[i].Id + '" /></td>' +
                                    '<td>' + jsonData.rows[i].MenuCode + '</td>' +
                                    '<td>' + jsonData.rows[i].MenuName + '</td>' +
                                    '<td>' + jsonData.rows[i].ParentName + '</td>' +
                                    '<td>' + jsonData.rows[i].AppName + '</td>' +
                                      '<td>' + (jsonData.rows[i].IsLeaf == 1 ? '是' : '否') + '</td>' +
    ...
    $("#btnAdd").click(function () {
        var retval = winModalFullScreen("MenuForm", false, 800, 600);
        PageClick(1);
    });
    ...
    $("#btnModify").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            var retval = winModalFullScreen("MenuForm?key=" + checkenodevalue, false, 800, 600);
            PageClick(1);
        }
    });
    ...
    $("#btnValid").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            InvokeAction.InvokeJsonAction('SysMgr', 'UpdateMenuValid', param, function (jsonData) {
                PageClick(1);
                alert(jsonData);
    ...
    $("#btnRemove").click(function () {
        ...
        if (CheckBoxOnly("chkSelect")) {
            if (confirm("确定要删除该信息吗?")) {
                ...
                InvokeAction.InvokeJsonAction('SysMgr', 'DeleteMenu', param, function (jsonData) {
                    PageClick(1);
                    alert(jsonData);
                });
            }
