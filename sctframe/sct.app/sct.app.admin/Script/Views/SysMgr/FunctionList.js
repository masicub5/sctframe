$(function () {
    ...
    PageClick = function (pageclickednumber) {
        ...
        InvokeAction.InvokeJsonAction('SysMgr', 'ListFunction', param, function (jsonData) {
            ...
            if (jsonData != null && jsonData.rows != null && jsonData.rows.length > 0) {
                for (i = 0 ; i < jsonData.rows.length; i++) {
                    ...
                    htmllist += '<td><input type = "checkbox" name ="chkSelect"  value="' + jsonData.rows[i].Id + '" /></td>' +
                                    '<td>' + jsonData.rows[i].FunctionCode + '</td>' +
                                    '<td>' + jsonData.rows[i].FunctionName + '</td>' +
                                    '<td>' + jsonData.rows[i].FunctionIcon + '</td>' +
    ...
    $("#btnAdd").click(function () {
        var retval = winModalFullScreen("FunctionForm", false, 800, 600);
        PageClick(1);
    });
    ...
    $("#btnModify").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            var retval = winModalFullScreen("FunctionForm?key=" + checkenodevalue, false, 800, 600);
            PageClick(1);
        }
    });
    ...
    $("#btnValid").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            InvokeAction.InvokeJsonAction('SysMgr', 'UpdateFunctionValid', param, function (jsonData) {
                PageClick(1);
                alert(jsonData);
    ...
    $("#btnRemove").click(function () {
        ...
        if (CheckBoxOnly("chkSelect")) {
            if (confirm("确定要删除该信息吗?")) {
                ...
                InvokeAction.InvokeJsonAction('SysMgr', 'DeleteFunction', param, function (jsonData) {
                    PageClick(1);
                    alert(jsonData);
                });
            }
