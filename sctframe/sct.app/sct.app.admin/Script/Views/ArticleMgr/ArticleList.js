$(function () {
    ...
    PageClick = function (pageclickednumber) {
        var titile = $("#txtQTitle").val();
        var articlecatalogid = $("#drplArticleCatalog").val();
        var isvalid = $("#drplIsvalid").val();
        param = { titile: titile, articlecatalogid: articlecatalogid, isvalid: isvalid, pagenumber: pageclickednumber, pagesize: pagesize };

        InvokeAction.InvokeJsonAction('ArticleMgr', 'ListArticle', param, function (jsonData) {
            ...
            if (jsonData != null && jsonData.rows != null && jsonData.rows.length > 0) {
                for (i = 0 ; i < jsonData.rows.length; i++) {
                    ...
                    htmllist += '<td><input type = "checkbox" name ="chkSelect"  value="' + jsonData.rows[i].Id + '" /></td>' +
                                    ...
                                    '<td>' + jsonData.rows[i].ArticleType + '</td>' +
                                    '<td>' + FormatDateString(jsonData.rows[i].FormDate) + '</td>' +
    ...
    $("#btnAdd").click(function () {
        var retval = winModalFullScreen("ArticleForm", false, 800, 600);
        PageClick(1);
    });
    ...
    $("#btnModify").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            var retval = winModalFullScreen("ArticleForm?key=" + checkenodevalue, false, 800, 600);
            PageClick(1);
        }
    });
    ...
    $("#btnValid").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            InvokeAction.InvokeJsonAction('ArticleMgr', 'UpdateArticleValid', param, function (jsonData) {
                PageClick(1);
                alert(jsonData);
    ...
    $("#btnRemove").click(function () {
        ...
        if (CheckBoxOnly("chkSelect")) {
            if (confirm("确定要删除该信息吗?")) {
                ...
                InvokeAction.InvokeJsonAction('ArticleMgr', 'DeleteArticle', param, function (jsonData) {
                    PageClick(1);
                    alert(jsonData);
                });
            }
