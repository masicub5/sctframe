$(function () {
    ...
    PageClick = function (pageclickednumber) {
        ...
        var parentid = $("#drplArticleCatalog").val();
        var iscolumn = $("#drplIscolumn").val();
        var isvalid = $("#drplIsvalid").val();
        param = { name: name, parentid: parentid, isvalid: isvalid, iscolumn: iscolumn, pagenumber: pageclickednumber, pagesize: pagesize };

        InvokeAction.InvokeJsonAction('ArticleMgr', 'ListArticleCatalog', param, function (jsonData) {
            ...
            if (jsonData != null && jsonData.rows != null && jsonData.rows.length > 0) {
                for (i = 0 ; i < jsonData.rows.length; i++) {
                    ...
                    htmllist += '<td><input type = "checkbox" name ="chkSelect"  value="' + jsonData.rows[i].Id + '" /></td>' +
                                    '<td>' + jsonData.rows[i].Code + '</td>' +
                                    '<td>' + jsonData.rows[i].Name + '</td>' +
                                    '<td>' + (jsonData.rows[i].IsColumn == 1 ? '是' : '否') + '</td>' +
    ...
    $("#btnAdd").click(function () {
        var retval = winModalFullScreen("ArticleCatalogForm", false, 800, 600);
        PageClick(1);
    });
    ...
    $("#btnModify").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            var retval = winModalFullScreen("ArticleCatalogForm?key=" + checkenodevalue, false, 800, 600);
            PageClick(1);
        }
    });
    ...
    $("#btnValid").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            InvokeAction.InvokeJsonAction('ArticleMgr', 'UpdateArticleCatalogValid', param, function (jsonData) {
                PageClick(1);
                alert(jsonData);
    ...
    $("#btnRemove").click(function () {
        ...
        if (CheckBoxOnly("chkSelect")) {
            if (confirm("确定要删除该信息吗?")) {
                ...
                InvokeAction.InvokeJsonAction('ArticleMgr', 'DeleteArticleCatalog', param, function (jsonData) {
                    PageClick(1);
                    alert(jsonData);
                });
            }
