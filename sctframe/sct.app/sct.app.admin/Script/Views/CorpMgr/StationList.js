$(function () {
    ...
    PageClick = function (pageclickednumber) {
        ...
        var parentid = $("#drplStation").val();
        var companyid = $("#drplCompany").val();
        var departmentid = $("#drplDepartment").val();
        var isvalid = $("#drplIsvalid").val();
        param = { name: name, companyid: companyid, departmentid: departmentid, parentid: parentid, isvalid: isvalid, pagenumber: pageclickednumber, pagesize: pagesize };

        InvokeAction.InvokeJsonAction('CorpMgr', 'ListStation', param, function (jsonData) {
            ...
            if (jsonData != null && jsonData.rows != null && jsonData.rows.length > 0) {
                for (i = 0 ; i < jsonData.rows.length; i++) {
                    ...
                    htmllist += '<td><input type = "checkbox" name ="chkSelect"  value="' + jsonData.rows[i].Id + '" /></td>' + 
                                    '<td>' + jsonData.rows[i].StationName + '</td>' +
                                    '<td>' + jsonData.rows[i].ParentName + '</td>' +
                                    '<td>' + jsonData.rows[i].DepartmentName + '</td>' +
                                    '<td>' + jsonData.rows[i].CompanyName + '</td>' +
    ...
    $("#btnAdd").click(function () {
        var retval = winModalFullScreen("StationForm", false, 800, 600);
        PageClick(1);
    });
    ...
    $("#btnModify").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            var retval = winModalFullScreen("StationForm?key=" + checkenodevalue, false, 800, 600);
            PageClick(1);
        }
    });
    ...
    $("#btnValid").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            InvokeAction.InvokeJsonAction('CorpMgr', 'UpdateStationValid', param, function (jsonData) {
                PageClick(1);
                alert(jsonData);
    ...
    $("#btnRemove").click(function () {
        ...
        if (CheckBoxOnly("chkSelect")) {
            if (confirm("确定要删除该信息吗?")) {
                ...
                InvokeAction.InvokeJsonAction('CorpMgr', 'DeleteStation', param, function (jsonData) {
                    PageClick(1);
                    alert(jsonData);
                });
            }
