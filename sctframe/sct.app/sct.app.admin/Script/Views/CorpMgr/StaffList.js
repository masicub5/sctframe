$(function () {
    ...
    PageClick = function (pageclickednumber) {
        ...
        InvokeAction.InvokeJsonAction('CorpMgr', 'ListStaff', param, function (jsonData) {
            ...
            if (jsonData != null && jsonData.rows != null && jsonData.rows.length > 0) {
                for (i = 0 ; i < jsonData.rows.length; i++) {
                    ...
                    htmllist += '<td><input type = "checkbox" name ="chkSelect"  value="' + jsonData.rows[i].Id + '" /></td>' +
                                    '<td>' + jsonData.rows[i].UserCode + '</td>' +
                                    '<td>' + jsonData.rows[i].UserName + '</td>' +
                                   '<td>' + (jsonData.rows[i].Sex == 1 ? '男' : '女') + '</td>' +
                                    '<td>' + jsonData.rows[i].Mobile + '</td>' +
                                    '<td>' + jsonData.rows[i].DeparmentName + '</td>' +
    ...
    $("#btnAdd").click(function () {
        var retval = winModalFullScreen("StaffForm", false, 800, 600);
        PageClick(1);
    });
    ...
    $("#btnModify").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            var retval = winModalFullScreen("StaffForm?key=" + checkenodevalue, false, 800, 600);
            PageClick(1);
        }
    });
    ...
    $("#btnValid").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            InvokeAction.InvokeJsonAction('CorpMgr', 'UpdateStaffValid', param, function (jsonData) {
                PageClick(1);
                alert(jsonData);
    ...
    $("#btnRemove").click(function () {
        ...
        if (CheckBoxOnly("chkSelect")) {
            if (confirm("确定要删除该信息吗?")) {
                ...
                InvokeAction.InvokeJsonAction('CorpMgr', 'DeleteStaff', param, function (jsonData) {
                    PageClick(1);
                    alert(jsonData);
                });
            }
