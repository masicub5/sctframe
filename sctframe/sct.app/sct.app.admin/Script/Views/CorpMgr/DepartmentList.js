$(function () {
    ...
    PageClick = function (pageclickednumber) {
        ...
        var parentid = $("#drplDepartment").val();
        var companyid = $("#drplCompany").val();
        var isvalid = $("#drplIsvalid").val();
        param = { name: name, parentid: parentid, companyid: companyid, isvalid: isvalid, pagenumber: pageclickednumber, pagesize: pagesize };

        InvokeAction.InvokeJsonAction('CorpMgr', 'ListDepartment', param, function (jsonData) {
            ...
            if (jsonData != null && jsonData.rows != null && jsonData.rows.length > 0) {
                for (i = 0 ; i < jsonData.rows.length; i++) {
                    ...
                    htmllist += '<td><input type = "checkbox" name ="chkSelect"  value="' + jsonData.rows[i].Id + '" /></td>' +
                                    '<td>' + jsonData.rows[i].DepartmentCode + '</td>' +
    ...
    $("#btnAdd").click(function () {
        var retval = winModalFullScreen("DepartmentForm", false, 800, 600);
        PageClick(1);
    });
    ...
    $("#btnModify").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            var retval = winModalFullScreen("DepartmentForm?key=" + checkenodevalue, false, 800, 600);
            PageClick(1);
        }
    });
    ...
    $("#btnValid").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            InvokeAction.InvokeJsonAction('CorpMgr', 'UpdateDepartmentValid', param, function (jsonData) {
                PageClick(1);
                alert(jsonData);
    ...
    $("#btnRemove").click(function () {
        ...
        if (CheckBoxOnly("chkSelect")) {
            if (confirm("确定要删除该信息吗?")) {
                ...
                InvokeAction.InvokeJsonAction('CorpMgr', 'DeleteDepartment', param, function (jsonData) {
                    PageClick(1);
                    alert(jsonData);
                });
            }
